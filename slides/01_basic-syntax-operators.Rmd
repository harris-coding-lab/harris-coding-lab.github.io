...
- Make small tweaks to the code to see if you understand how the code works - be hands-on!
<!--We will do things that produce errors and then talk about how to read the error and fix it.
-->
- Be lazy: usually someone else has had your problem and developed a solution.
...
google: "create a vector in R that has the same number repeated"
```{r}
c("a", "a", "a", "a")
rep("a", 4)
```
...
*** 
Creating vectors of counting numbers
```{r}
c(2, 3, 4, 5)
2:5
seq(2, 5)
...
Why does vector `a` look come out this way? Remember, `my_numbers` is a list of numbers from 1 to 6.
```{r}
a <- my_numbers + c(1, 2)
```
...
- work in a team of 2-4 on the tutorial
 - [https://harris-coding-lab.shinyapps.io/basic-syntax/] 
 - (link available on course webpage as well)

- if you get stuck as a team, add your name to the help list 
 - [bit.ly/coding_lab_qa]
