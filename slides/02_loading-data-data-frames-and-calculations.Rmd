...
```{r}
texas_housing_data <- get("txhousing") # txhousing is in tidyverse
...
```{r}
mutant_data <- texas_housing_data %>%
  mutate(mean_house_price = volume / sales,)
```
...
![](images/02_summarize.png){#id .class width=80% height=80%}
...
```{r}
annual_housing_prices <- texas_housing_data %>%
  group_by(year) %>%
  summarize(total_sales = sum(sales, na.rm = TRUE),
            total_volume = sum(volume, na.rm = TRUE),
            mean_house_price = total_volume / total_sales)
...
## two more dplyr functions you'll see in the workshop
...
## want to learn more about the `tidyverse`?
