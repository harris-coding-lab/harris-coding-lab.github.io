...
## installing and loading packages {.build}
...
- the underlying language is the same
- but our code will look distinctly `tidyverse`
...
Hint: `getwd()` shows your current working directory <!--point out that if they don't know what a directory structure looks like on their computer (windows) they can see it there -->.
...
One approach: Use select, storing each step as intermediate data.
...
- by default, the LHS is the *first argument* of the RHS function.
...
## Ceci est une `%>%`
We can chain together tidyverse functions to avoid make so many intermediate data frames!
```{r}
texas_housing_data %>%
...
`left_join()` | to combine datasets | preview in wkshp
