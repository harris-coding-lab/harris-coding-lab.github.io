...
## A Refresher on Functions in R

You've already been using functions throughout this lab, ranging from `read_csv()` to `group_by()`. These are all functions, programs that take an input and deliver an output. In this sense, they're just like a math function, such as $f(x) = x^2$. In R, we can easily write our own functions:
...
All we have to do is come up with a name, `squared`, calling the function command, telling it what input to expect, `x`. Then, our instructions go inside the curly braces—take the input, and square it. R functions will return, or give the output, of the last object called. So in this case you don't have to do anything else. Try it out by running the code below:

```{r}
squared(2)
```

Sometimes, you want to specify exactly what gets returned, in which case you can tell R by using `return()`:
...
There's more that goes into functions, but if there's one thing you should take away from this is that what goes inside the curly brackets is 97% the same as the code you've already been writing. The only thing you're doing by making it into a function is saving all that code in a compact form that you can reuse. If you approach things this way you'll realize there's nothing complicated about functions themselves.

Functions are super helpful for code that you expect to use many times. As a guideline of sorts, if you have to copy and paste a piece of code three or more times and it's not something that you'll loop once and forget about  (you'll learn about loops in another lab), you should probably write a function for it. As you become better and better at coding, you'll amass a collection of useful functions you can reuse for other projects! 
...
So finally, write a function called `monte_carlo_samples` that takes a number of observations, a number of replications, and a true mean, then replicates your `get_mean_and_sd_from_random_sample()` function `B` times and outputs a tibble with that many sample means and sample standard deviations.
