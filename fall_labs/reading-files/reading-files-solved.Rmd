...
**Answer:** You need `filter()` and `select()`, and the code would be: 
...
**Answer:** You need `group_by()` and `summarize()`. The dataset would have as many counties times years you have, and the code would be:
...
```{r subset3, echo=TRUE, eval=FALSE}
filter(student_loan_debt, row_condition) %>% 
  select(column_name)
```
...
```{r filter2}
filter(student_loan_debt, per_capita_student_debt >= 5000, year == 2008)
```
...
How about the mean per capita debt by year? Let's call this one `student_loan_debt_by_year`.
...
**Answer:** The minimum per capita debt is 1510, and the mean debt is 3676
...
student_loan_debt_by_year_weighted <- joined_data %>%
  ...
  summarize(population = sum(population, na.rm = TRUE),
            annual_student_debt = sum(total_student_debt, na.rm = TRUE)) 
