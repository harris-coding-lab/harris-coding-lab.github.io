...
**Answer:** Each number in the vector `random_numbers` did not change, because it's particular value is saved in our global environment from previous. However if we re-ran the above chunk titled `{r random-numbers-create}` we would see each number in `random_numbers` change.
