...
The code returns an error because the package names need to be in quotes:
...
We use `<-` for assigning variables in R. We can assign all sorts of objects to names, including data sets. All we have to do is add `big_storms <-` at the beginning of the code; that will tell R to take the output of the code and assign it to the name `big_storms`:
...
Most of you probably saw the following error when you ran the code:
...
The solution is to install and load the `maps` package:
...
Looking at the code, you'll notice that the `xlim` and `ylim` parameters look like the longitude and latitude positions where the graph cuts off. Changing these limits will also change the part of the world the graph shows. You can play around with these numbers until you have a graph you like, such as:

```{r}
ggplot(aes(x = long, y = lat, color = name), data = big_storms) +
  geom_path() + 
  borders("world") + 
  coord_quickmap(xlim = c(-130, 0), ylim = c(0, 50))
```
...
`covid_data` appears to be tidy. Each row represents one week of deaths for one race/ethnicity within one state.
...
`summary(covid_data)` shows us a summary of each variable within the `covid_data` data frame. When we look specifically at `n_deaths`, we see that almost all of the data has two or three digits, but for some reason the maximum value has five digits. That's quite an outlier!
...
The reason we have outliers in the data is that, while most of the data is at the state level, the CDC decided to also include aggregated data for the entire country as if it were also a state.
...
We run the following code and get a new graph:
...
Notice that the new graph uses the variable names as the axis labels, whereas the original graph had nicer descriptive labels. This implies that `labs()` sets labels for the graph that override the default behavior, which is to use the names of the variables being graphed.
...
covid_data %>%
  ...
  filter(race_ethnicity %in% 
    c("Hispanic", "Non-Hispanic White", "Non-Hispanic Black", "Non-Hispanic Asian")) %>%
  ggplot(aes(x = week_no, y = n_deaths, color = race_ethnicity)) +
  geom_line()  +
labs(y = "total number of deaths", 
     x = "week since January 1, 2020", 
     title = "Racial disparities of Covid-19 (USA, 2020)", 
     color = "" )
```
...
We made a typo in the initial assignment; rather than `filter(week_no > 30) %>%`, it should be `filter(week_no < 30) %>%`; that is to say, we want to filter the data set so it only includes data from *before* week 30, not *after* week 30.
