...
1. For the problem above, your groupmate tried the following code. Explain why the frequencies are all 1.^[Hint: This is a lesson about `group_by()`!]
    ...
    As explained in the linked stackoverflow post, the last grouping variable is peeled off after the summarise function. So, if you calculate frequencies within the summarize function, each race will be treated as a single group, and its frequency must be 1. However, if you calculate frequencies after the summerise function, the whole data will be treated as one group.
