...
4. In your script, load `haven` and `readxl`. Notice that if you had to restart R right now. You could reproduce the entire warm-up by running the script. We strive for reproducibility by keeping the code we want organized in scripts or `Rmd`s.
...
After filtring the `french_data` to between 1960 and 1970, we can see that there are missing values in the `value` column thus explaining the gaps in the plot
...
The data doesn't seem to allign with other sources from Google.
