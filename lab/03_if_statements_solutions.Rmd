...
```{r}
mrc_data %>% 
  filter(1330 <= sat_avg_2013, sat_avg_2013 <= 1530) %>%
...
mrc_data %>% 
  ...
  summarize(min(sat_avg_2013),
            max(sat_avg_2013))
```
Simple way to figure out if `between()` uses `<` or `<-`, in order words, if `between()` includes endpoints, is to look at those endpoints. You can do that with `summarize()`. Here you can see that if we filter with `<=` and filter with `between()`, we get the same max and min values for `sat_avg_2013`.
...
bff_super_awesome_college_list <- mrc_data %>%
  mutate(abdul = ifelse(between(sat_avg_2013, 1330, 1530) & 
         ...
         stephen = ifelse(between(sat_avg_2013, 1350, 1550) &
                            (public == 0), TRUE, FALSE),
         nancy = ifelse(between(sat_avg_2013, 1490, 1690) &
                          (region == 2), TRUE, FALSE),
         rei = ifelse(between(sat_avg_2013, 1390, 1590) &
...
*Solution*
Yes, the school list will change if we replace all `NA`s with `TRUE`. The list will not change if we replace `NA`s with `FALSE` since we are using the OR (|) operater, and this operater evaluates to `TRUE` so long as at least one statment is `TRUE`.
...
bff_super_awesome_college_list %>%
  mutate(num_friends_accept = abdul + stephen + nancy + rei + casey,
         rank = case_when(num_friends_accept == 5 ~ "1",
                          ...
                          num_friends_accept == 4 ~ "2",
                          num_friends_accept == 5 ~ "1",
                          num_friends_accept == 0 ~ "Unranked")) %>%
  arrange(rank) %>%
  select(name, num_friends_accept, rank)
```
Here, we made use of the fact that booleans (`TRUE` and `FALSE`) can be treated like `1`s and `0`s. Since `abdul`, `stephen`, `nancy`, `rei`, and `casey` are just logical vectors (vectors full of `TRUE`s and `FALSE`s), if we add across the rows which represent schools, we get the number of friends who would accept going to that school. Then, we can use `case_when` to assign the rankings according to how many friends would accept going to that school. The last your lines (`arrange()` and `select()`) are not necessary to complete this task but make the outcome simpler to assess and easier for you to see if you've done the task correctly.
